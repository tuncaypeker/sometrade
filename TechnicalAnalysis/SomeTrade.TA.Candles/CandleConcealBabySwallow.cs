...
namespace SomeTrade.Candles
{
    public class CandleConcealBabySwallow : CandleIndicator
    {
        ...
        public CandleResult Compute(int startIdx, int endIdx)
        {
            ...
            while (i < startIdx)
            {
                _shadowVeryShortPeriodTotal[3] += GetCandleRange(CandleSettingType.ShadowVeryShort, i - 3);
        ...
        public override bool GetPatternRecognition(int i)
        {
            bool isConcealBabySwallow =
                ...
                // 1st: marubozu
                GetLowerShadow(i - 3) <
                GetCandleAverage(CandleSettingType.ShadowVeryShort, _shadowVeryShortPeriodTotal[3], i - 3) &&
                GetUpperShadow(i - 3) <
                GetCandleAverage(CandleSettingType.ShadowVeryShort, _shadowVeryShortPeriodTotal[3], i - 3) &&
                // 2nd: marubozu
                GetLowerShadow(i - 2) <
                GetCandleAverage(CandleSettingType.ShadowVeryShort, _shadowVeryShortPeriodTotal[2], i - 2) &&
                GetUpperShadow(i - 2) <
                GetCandleAverage(CandleSettingType.ShadowVeryShort, _shadowVeryShortPeriodTotal[2], i - 2) &&
                ...
                GetUpperShadow(i - 1) >
                GetCandleAverage(CandleSettingType.ShadowVeryShort, _shadowVeryShortPeriodTotal[1], i - 1) &&
