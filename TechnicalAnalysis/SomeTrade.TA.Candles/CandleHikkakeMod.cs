...
namespace SomeTrade.Candles
{
    public class CandleHikkakeMod : CandleIndicator
    {
        ...
        public CandleResult Compute(int startIdx, int endIdx)
        {
            ...
            int nearTrailingIdx = startIdx - 3 - GetCandleAvgPeriod(CandleSettingType.Near);
            
            int i = nearTrailingIdx;
            while (i < startIdx - 3)
            {
                _nearPeriodTotal += GetCandleRange(CandleSettingType.Near, i - 2);
                i++;
            }
            ...
            while (i < startIdx)
            {
                ...
                _nearPeriodTotal +=
                    GetCandleRange(CandleSettingType.Near, i - 2) -
                    GetCandleRange(CandleSettingType.Near, nearTrailingIdx - 2);
        ...
        public override bool GetPatternRecognition(int i)
        {
            bool patternRecognition =
                ...
                (
                    ...
                    _close[i - 2] <= _low[i - 2] + GetCandleAverage(CandleSettingType.Near, _nearPeriodTotal, i - 2) ||
                    // (bear) 4th: higher high and higher low
                    _high[i] > _high[i - 1] && _low[i] > _low[i - 1] &&
                    // (bull) 2nd: close near the top
                    _close[i - 2] >= _high[i - 2] - GetCandleAverage(CandleSettingType.Near, _nearPeriodTotal, i - 2)
                );
        ...
        {
            return Max(1, GetCandleAvgPeriod(CandleSettingType.Near)) + 5;
        }
    }
}
