...
namespace SomeTrade.Candles.Common
{
    public abstract class CandleIndicator
    {
        ...
        protected double GetRealBody(int index)     => Abs(_close[index] - _open[index]);
        protected double GetUpperShadow(int index)  => _high[index] - (_close[index] >= _open[index] ? _close[index] : _open[index]);
        protected double GetLowerShadow(int index)  => (_close[index] >= _open[index] ? _open[index] : _close[index]) - _low[index];
        protected double GetHighLowRange(int index) => _high[index] - _low[index];
        
        protected bool GetRealBodyGapUp(int index2, int index1)     => Min(_open[index2], _close[index2]) > Max(_open[index1], _close[index1]);
        protected bool GetRealBodyGapDown(int index2, int index1)   => Max(_open[index2], _close[index2]) < Min(_open[index1], _close[index1]);
        protected bool GetCandleGapUp(int index2, int index1)       => _low[index2] > _high[index1];
        protected bool GetCandleGapDown(int index2, int index1)     => _high[index2] < _low[index1];

        protected int GetCandleColor(int index)     => _close[index]>= _open[index] ? 1 : -1;
        protected int GetCandleMaxAvgPeriod(params CandleSettingType[] candleSettingTypes) => candleSettingTypes.Select(GetCandleAvgPeriod).Aggregate(Max);
    }
}
