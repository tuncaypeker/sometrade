namespace SomeTrade.TA.Indicators.KivancOzbilgic
{
    ...
    /// link:
    ///    https://tr.tradingview.com/script/nJnz4FX3/
    ...
    //      creator John EHLERS
    //
    ///     study("Inverse Fisher Transform on CCIv2", shorttitle="IFTCCIv2")
    ...
    /// </summary>
    public class IFTCCI2
    {
        public static double[] Calculate(double[] closeArray, double[] highArray, double[] lowArray, int cciLength, int wmaLength)
        {
            ...
            var cciArray = TA.CCI.Calculate(highArray, lowArray, closeArray, cciLength);
            var v1 = new double[highArray.Length];
            for (int i = 0; i < highArray.Length; i++)
                v1[i] = 0.1 * (cciArray[i]/4);

            var v2 = TA.WMA.Calculate(v1, wmaLength);

            //validation
            var INV = new double[highArray.Length];
