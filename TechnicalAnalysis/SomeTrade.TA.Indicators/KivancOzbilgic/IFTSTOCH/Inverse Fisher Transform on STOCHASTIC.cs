namespace SomeTrade.TA.Indicators.KivancOzbilgic
{
    ...
    /// link:
    ///    https://tr.tradingview.com/script/WikDwOZC/
    ...
    /// </summary>
    public class IFTSTOCH
    {
        public static double[] Calculate(double[] closeArray, double[] highArray, double[] lowArray, int stochLength, int wmaLength)
        {
            ...
            var stochArray = Stochastic.Calculate(closeArray, highArray, lowArray, stochLength).K;
            var v1 = new double[highArray.Length];
            for (int i = 0; i < highArray.Length; i++)
                v1[i] = 0.1 * (stochArray[i] - 50);
