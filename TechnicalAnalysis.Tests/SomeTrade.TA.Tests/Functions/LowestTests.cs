using SomeTrade.TA.Tests.Utilities;
using System.Linq;
using Xunit;

namespace SomeTrade.TA.Tests.Functions
{
    public class LowestTests
    {
        [Fact]
        public void Execute()
        {
            // Arrange
            var candles = _Data_BITSTAMP_BTC_USD_1Mo.Data;
            double[] high = candles.Select(x => x.High).ToArray();
            double[] low = candles.Select(x => x.Low).ToArray();
            double[] close = candles.Select(x => x.Close).ToArray();
            double[] volume = candles.Select(x => x.Volume).ToArray();
            var dateTimes = candles.Select(x => x.Time).ToList();

            // Act
            var actualResult = Lowest.Calculate(close, 40);

            // Assert
            var i3 = dateTimes.FindIndex(x => x.Year == 2016 && x.Month == 2); //12.43
            Assert.True(actualResult[i3] > 12 && actualResult[i3] < 13);

            var i1 = dateTimes.FindIndex(x => x.Year == 2017 && x.Month == 7); //216.90
            Assert.True(actualResult[i1] > 216 && actualResult[i1] < 217);

            var i2 = dateTimes.FindIndex(x => x.Year == 2020 && x.Month == 5);//1070.31
            Assert.True(actualResult[i2] > 1070 && actualResult[i2] < 1071);
        }
    }
}
